---
title: Héctor Gómez
type: entity
tags:
- risk-management
- derivatives
- research-team
created: 2026-04-21
updated: '2026-04-21'
sources:
- sources/mcp-research-team-paper-assignment-profiles.md
confidence: high
---

# Héctor Gómez

Héctor Gómez is the **Risk & Derivatives Lead** within the MCP Research Team, responsible for curating research on volatility surfaces, commodities, and macro-FX crossovers. His work spans risk management, term structure analysis, implied volatility surfaces, and commodity pricing models, with a focus on factor uncertainty, mean reversion, and geopolitical risk [[sources/mcp-research-team-paper-assignment-profiles.md]]. As the natural "curator" in the research pipeline, he acts as the first filter for new papers and is the default owner for topics including derivatives pricing, vol surfaces, risk models, backtesting, and FX hedging [[sources/mcp-research-team-paper-assignment-profiles.md]].

Gómez’s role also involves overseeing research that intersects macroeconomic factors with commodities and FX markets, making him a key contact for studies on long/short signals and options market information content. The team’s quick routing cheat-sheet explicitly directs derivative-related research to him, emphasizing his expertise in term structure, volatility, and macro-FX crossovers [[sources/mcp-research-team-paper-assignment-profiles.md]]. His work is closely linked to [[entities/jp-sada]] and [[entities/carlos]], who handle infrastructure and LLMs-for-investing topics, respectively, while his technical focus areas overlap with concepts like [[concepts/agentic-ai]] and [[concepts/data-leakage]].

## Sources
- [[sources/mcp-research-team-paper-assignment-profiles.md]]
